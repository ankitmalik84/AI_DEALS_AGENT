...
MODEL_DIR = "/root/hf-cache/"  # Changed to absolute path
...
class Pricer:
    ...
    def setup(self):
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(BASE_MODEL)  # Changed to use model ID
        self.tokenizer.pad_token = self.tokenizer.eos_token
        self.tokenizer.padding_side = "right"
        
        self.base_model = AutoModelForCausalLM.from_pretrained(
            BASE_MODEL,  # Changed to use model ID
            quantization_config=quant_config,
            device_map="auto"
        )
    
        self.fine_tuned_model = PeftModel.from_pretrained(self.base_model, FINETUNED_MODEL, revision=REVISION)  # Changed to use model ID
